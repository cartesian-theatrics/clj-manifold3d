...
(defn texture-options
  ...
  [{:keys [mapping name origin normal u-direction size pixel-size uv-rect opacity axes scale offset
           depth-map depth-scale depth-offset depth-fade depth-boundary] :as options}]
  (when-let [unknown (seq (remove #{:mapping :name :origin :normal :u-direction :size :pixel-size :uv-rect
                                   :opacity :axes :scale :offset :depth-map :depth-scale :depth-offset :depth-fade :depth-boundary}
                                 (keys options)))]
  ...
  (let [mapping (or mapping :geodesic) opacity (or opacity 1)
        ...
        scale (if (number? scale) [scale scale] (or scale [1 1]))
        axes (mapv #(get {:x 0 :y 1 :z 2} % %) (or axes [:x :z]))
        pixel (or pixel-size 0)]
    (when-not (and (#{:geodesic :planar} mapping) (finite? opacity) (<= 0 opacity 1)
    ...
    (when (and (= mapping :planar) depth-map) (throw (ex-info "Depth requires :geodesic mapping" {})))
