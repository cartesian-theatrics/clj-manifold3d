...
(async()=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>document.querySelector('.result-label')?.textContent.includes('Scene ·') ||
    [...document.querySelectorAll('.result-label')].some(e=>e.textContent.includes('Scene ·')) ||
    document.querySelector('.block-result[data-status="error"]'),null,{timeout:600000});
  ...
  const model=page.locator('.solid-preview');await model.scrollIntoViewIfNeeded();
  await page.waitForFunction(()=>document.querySelector('.solid-preview')?.dataset.loaded==='true',null,{timeout:120000});
