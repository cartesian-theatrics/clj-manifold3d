...
(defn- snapshot [model]
  (let [mesh (m/get-mesh-gl model)
        ...
        faces (vec (.toIntArray (.faceID mesh)))
        runs (vec (.toIntArray (.runIndex mesh)))
        originals (vec (.toIntArray (.runOriginalID mesh)))]
    {:rows rows
...
(defn- prepare [{:keys [corners] :as triangle}]
  (let [[a b c] corners
    ...
    (assoc triangle :ab ab :ac ac :normal normal :squared-area squared-area
           ...
           :longest-edge (Math/sqrt (max (dot ab ab) (dot ac ac)
...
(defn- interpolation-error
  ...
  (when (pos? squared-area)
    (let [[a b c] corners
          ...
          area (Math/sqrt squared-area)
          plane-distance (/ (Math/abs (double (dot ap normal))) area)]
            ...
            (fn [reports [kind sample]]
              (let [direct-error (best-error candidates sample position-tolerance)
                    ...
                    error (if fallback?
                            ...
                            direct-error)
                    failed? (or (not (Double/isFinite error)) (> error uv-tolerance))
                    new? (and (= :corner kind)
                ...
                (update reports owner
                        (fn [stats]
                          (cond-> (-> stats
                            ...
                            (not (Double/isFinite error)) (update :missing-source inc)
